...
def registration_page_function():
    ...
    # Set to store generated user IDs
    ...
    def generate_captcha_text():      
        # Define characters for CAPTCHA generation (letters and digits)
        captcha_characters = string.ascii_letters + string.digits
        
        # Generate a random sequence of 6 characters from the defined character set
    ...
    def generate_captcha_image():
        global captcha_text
        # Create an ImageCaptcha object with specified height and width
        captcha_obj = ImageCaptcha(height=100, width=250)
        
        # Generate a new CAPTCHA text
        captcha_text = generate_captcha_text()
        
        # Generate CAPTCHA image based on the generated text
        captcha_image = captcha_obj.generate(captcha_text)
        captcha_image = Image.open(captcha_image)  # Open the generated image
    ...
    def update_captcha_image():
        ...
        captcha_label.image = photo  # Keep a reference to the image object to prevent it from being garbage collected
    ...
    Password_shown_check = ctk.CTkCheckBox(master=registration_page, text="Show Password", command=show_hide_password, font=(font_style, 11))
    Password_shown_check.place(x=48, y=425)
    Password_shown_check.configure(checkbox_width=15, checkbox_height=15)    
    ...
    captcha_text_label.place(x=48, y=450)
    
    captcha_entry = ctk.CTkEntry(registration_page, width=150)
    captcha_entry.place(x=48, y=475)
    
    # Bind the <FocusIn> event to the captcha_entry widget
    captcha_entry.bind("<FocusIn>", lambda event: update_captcha_image())
    
    Submit_button = ctk.CTkButton(registration_page, text="Submit", command=validate_captcha)
    Submit_button.place(x=48, y=510)
    
    
    # Label to display the CAPTCHA image
    captcha_label = ctk.CTkLabel(registration_page, text="")  
    captcha_label.place(x=230, y=460)
    ...
    register_button.place(x=48, y=550)  
...
def system_user_dashboard_function():    
    ...
    def navigate_to_edit_page():
        destroy_current_page_widgets()
        app.update()
        system_user_edit_button_function()


    # Forward to delete account page
    ...
    def get_user_information(user_id):
        with open("user_accounts.csv", "r", newline="") as csvfile:
            ...
            for row in reader:
                # Check if the user ID in the current row matches the user_id parameter
                if row[0] == user_id:
                    # If a matching user is found, create a dictionary containing their information
                        user_info = {
                            "User ID": row[0],
                            "First Name": row[1],
                            "Last Name": row[2],
    ...
    def get_profile_picture_path(user_id):
        # Define the directory where user pictures are stored
        ...
        for ext in file_extensions:
            # Create full path of picture using User ID as file name 
        ...
        # If no picture file is found for the user, return None    
        return None
    ...
    def system_user_home_button_function():
        ...
        # Function to display user accounts of current month
        def display_user_accounts_for_month():
            ...
            display_window.geometry("1300x300")
    ...
    def system_user_edit_button_function():
        # Function to display user information based on the provided user ID
        def showw_user_info(user_id):
            # Read the CSV file to find the user's information
            with open("user_accounts.csv", "r") as csvfile:
                reader = csv.reader(csvfile)
                next(reader)  # Skip the header row

                # Initialize a variable to check if user ID exists
                user_found = False

                # Iterate through each row in the CSV file
                for row in reader:
                    # Check if the user ID in the current row matches the entered user ID
                    if row[0] == user_id:
                        # Set user_found flag to True
                        user_found = True
                        
                        # Autofill the entry fields with existing user information
                        First_name_entry.insert(0, row[1])
                        Last_name_entry.insert(0, row[2])
                        Phone_number_entry.insert(0, row[4])
                        Address_entry.insert(0, row[5])
                        Email_entry.insert(0, row[6])
                        Role_entry.insert(0, row[8])

                        # Break the loop since user information is found
                        break

                # If user ID does not exist, show an error message
                if not user_found:
                    messagebox.showinfo("Error", "User ID not found.")
    
    
        # Function to handle changes made by the system user
        def done_button_function(user_id):
            first_name = First_name_entry.get()
            last_name = Last_name_entry.get()
            phone_number = Phone_number_entry.get()
            ...
            if not is_valid_phone_number(phone_number):
                ...
                if user_index is not None:
                    # Update the user's information in the list of rows
                    rows[user_index][1] = first_name
                    rows[user_index][2] = last_name
                    rows[user_index][4] = phone_number
                    rows[user_index][5] = address
                    rows[user_index][6] = email
                    rows[user_index][8] = role
        ...
        edit_page = ctk.CTkFrame(master=system_user_dashboard, width=550, height=600)
        edit_page.place(x=150, y=0)
        
        pic_path = tk.StringVar()
        pic_path.set("")
        
        User_ID_label = ctk.CTkLabel(edit_page, text="First Enter User ID to modify data", font=(font_style, 12))
        User_ID_label.place(x=250, y=30)
        
        User_ID_entry = ctk.CTkEntry(edit_page, width=150)
        User_ID_entry.place(x=250, y=55) 
        
        enter_button = ctk.CTkButton(edit_page, text="Search", command=lambda: showw_user_info(User_ID_entry.get()), font=(font_style, 12))
        enter_button.place(x=250, y=100)       
        
        profile_picture_path = get_profile_picture_path(User_ID_entry.get())
        

        # Create profile picture frame
        profile_picture = ctk.CTkFrame(edit_page, width=105, height=105, border_color="black", border_width=2)
        ...
        First_name_label = ctk.CTkLabel(edit_page, text="First Name *", font=(font_style, 12))
        First_name_label.place(x=40, y=150)
        First_name_entry = ctk.CTkEntry(edit_page, width=230)
        First_name_entry.place(x=40, y=175)

        Last_name_label = ctk.CTkLabel(edit_page, text="Last Name *", font=(font_style, 12))
        Last_name_label.place(x=280, y=150)
        Last_name_entry = ctk.CTkEntry(edit_page, width=230)
        Last_name_entry.place(x=280, y=175)

        Role_label = ctk.CTkLabel(edit_page, text="Role", font=(font_style, 12))
        Role_label.place(x=40, y=210)
        Role_entry = ctk.CTkEntry(edit_page, width=230)
        Role_entry.place(x=40, y=235)

        Phone_number_label = ctk.CTkLabel(edit_page, text="Phone Number (09xxxxxxxxx) *", font=(font_style, 12))
        Phone_number_label.place(x=280, y=210)
        Phone_number_entry = ctk.CTkEntry(edit_page, width=230)
        Phone_number_entry.place(x=280, y=235)

        Address_label = ctk.CTkLabel(edit_page, text="Address *", font=(font_style, 12))
        Address_label.place(x=40, y=270)
        Address_entry = ctk.CTkEntry(edit_page, width=330)
        Address_entry.place(x=40, y=295)

        Email_label = ctk.CTkLabel(edit_page, text="Email *", font=(font_style, 12))
        Email_label.place(x=40, y=330)
        Email_entry = ctk.CTkEntry(edit_page, width=330)
        Email_entry.place(x=40, y=355)
        
        # Create done function button
        done_button = ctk.CTkButton(edit_page, text="Done", command=lambda: done_button_function(User_ID_entry.get()),
                                    font=(font_style, 12))
        done_button.place(x=40, y=540)


    # Function of delete account button in system user dashboard
    def system_user_delete_account_button_function():
        
        # Create delete account funtion by the provided user ID 
        ...
        # Create delete funtion button 
        delete_button = ctk.CTkButton(delete_page, text="Delete", 
                                      command=lambda: delete_account(search_user_ID_entry.get()), 
                                      font=(font_style, 12))
        delete_button.place(x=175, y=140)
    ...
    def system_user_logout_button_function():
        ...
        if confirm == "yes":
            # If user confirms logout, destroy the current dashboard and return to the login page
            system_user_dashboard.destroy()
            app.update()
            Login_page_function()  # Redirect to the login page
        else:
            # If user cancels logout, return to the system user dashboard (home page)
            system_user_dashboard_function()
    ...
    system_user_edit_button = ctk.CTkButton(system_user_menu, text="Edit", 
                                              command=navigate_to_edit_page,
                                              font=(font_style, 12))
    system_user_edit_button.place(x=5, y=230)
    ...
    system_user_delete_account_button = ctk.CTkButton(system_user_menu, text="Delete Account", 
                                                      ...
                                                      font=(font_style, 12))
    system_user_delete_account_button.place(x=5, y=270)
...
def end_user_dashboard_function(user_email):
    ...
    def get_user_information(user_email):
        with open("user_accounts.csv", "r", newline="") as csvfile:
            ...
            for row in reader:
                # Check if the email in the current row matches the user_email parameter
                # and if the user's role is "End User"
    ...
    def end_user_edit_button_function(user_email):
        # Function to autofill entry fields with existing user information
        def autofill_entry_fields():
            First_name_entry.insert(0, user_info["First Name"])
            Last_name_entry.insert(0, user_info["Last Name"])
            Date_of_birth_entry.delete(0, "end")  # Clear the current date from calendar
            Date_of_birth_entry.insert(0, user_info["Date of Birth"])
            Phone_number_entry.insert(0, user_info["Phone Number"])
            Address_entry.insert(0, user_info["Address"])
            Email_entry.insert(0, user_info["Email"])
            
                    
        # Function to handle the changes made by the user
        def change_button_function():
            ...
            # Validation Check
            if is_today_date_of_birth(date_of_birth):
                messagebox.showinfo("Error", "Date of birth cannot be today's date")
                ...
                if user_index is not None:
                    ...
                    rows[user_index][1] = first_name
                    rows[user_index][2] = last_name
                    rows[user_index][3] = date_of_birth
                    rows[user_index][4] = phone_number
                    rows[user_index][5] = address
                    rows[user_index][6] = email
        ...
        def handle_picture_upload(user_id):
            ...
            if file_path:
                ...
                if file_size_MB > max_file_size:
                    ...
                    error_message = messagebox.showinfo("Error", f"File size exceeds the maximum allowed size of {max_file_size} MB.")
                ...
                if file_extension.lower() not in [".jpg", ".png"]:
                    # Display error message and return
                    error_message = messagebox.showinfo("Invalid file type. Please upload a JPG file.")
                    return error_message
        ...
        # StringVar to store the picture path
        pic_path = tk.StringVar()
        pic_path.set("")
        ...
        # Check if profile picture path exists and create appropriate image
        if profile_picture_path:
            # Display the uploaded user's picture
            user_profile_pic = ctk.CTkImage(light_image=Image.open(profile_picture_path), 
                                            dark_image=Image.open(profile_picture_path), size=(105, 105))
        else:
            # Display default profile picture if path doesn't exist
        ...
        First_name_label = ctk.CTkLabel(edit_profile_page, text="First Name *", font=(font_style, 12))
        First_name_label.place(x=40, y=150)
        First_name_entry = ctk.CTkEntry(edit_profile_page, width=230)
        First_name_entry.place(x=40, y=175)

        Last_name_label = ctk.CTkLabel(edit_profile_page, text="Last Name *", font=(font_style, 12))
        Last_name_label.place(x=280, y=150)
        Last_name_entry = ctk.CTkEntry(edit_profile_page, width=230)
        Last_name_entry.place(x=280, y=175)

        Date_of_birth_label = ctk.CTkLabel(edit_profile_page, text="Date Of Birth *", font=(font_style, 12))
        Date_of_birth_label.place(x=40, y=210)
        Date_of_birth_entry = DateEntry(edit_profile_page, width=30, background='darkblue',
                                    foreground='white', borderwidth=2)
        Date_of_birth_entry.place(x=70, y=355)

        Phone_number_label = ctk.CTkLabel(edit_profile_page, text="Phone Number (09xxxxxxxxx) *", font=(font_style, 12))
        Phone_number_label.place(x=280, y=210)
        Phone_number_entry = ctk.CTkEntry(edit_profile_page, width=230)
        Phone_number_entry.place(x=280, y=235)

        Address_label = ctk.CTkLabel(edit_profile_page, text="Address 1 *", font=(font_style, 12))
        Address_label.place(x=40, y=270)
        Address_entry = ctk.CTkEntry(edit_profile_page, width=330)
        Address_entry.place(x=40, y=295)

        Email_label = ctk.CTkLabel(edit_profile_page, text="Email *", font=(font_style, 12))
        Email_label.place(x=40, y=330)
        Email_entry = ctk.CTkEntry(edit_profile_page, width=330)
        Email_entry.place(x=40, y=355)

        # bind existing info into the entry fields
        autofill_entry_fields() 
        
        # Create change function button 
        change_button = ctk.CTkButton(edit_profile_page, text="Done", command=change_button_function,
                                    font=(font_style, 12))
        change_button.place(x=40, y=400)
    ...
    def end_user_change_password_button_function(user_email):
        ...
        # Function to validate and update the user's password
        def confirm_button(): 
            ...
            if not (
                    is_present(current_password) or
                    is_present(new_password) or
                    is_present(confirm_new_password) 
            ):
                ...
                for i, row in enumerate(rows):
                    # Check if the email matches to get the user's index in CSV file 
                    if row[6] == user_email:  
                        # Get the user's index in CSV file 
                        user_index = i 
                        break

                if user_index is not None:
                    # Get the stored password 
                    stored_password = rows[user_index][7]  
        ...
        # Create change password page frame
        change_button_page = ctk.CTkFrame(master=end_user_dashboard, width=550, height=600)
        change_button_page.place(x=150, y=0)
        
        title_label = ctk.CTkLabel(change_button_page, text="CHANG PASSWORD", font=(font_style, 23, "bold"))
        title_label.place(x=130, y=30)        
        
        # Create password rule label
        password_rule = ("• Password must be at least 8 characters long.\n"
                        "• Password must contain letters.\n"
                        "• Password must contain digits.\n"
                        "• Password must contain at least one special character.\n")
                        
        
        password_rule_label = ctk.CTkLabel(change_button_page, text=password_rule, font=(font_style, 12), text_color="red")
        password_rule_label.place(x=80, y=70)
        
        # Create announcement on changing password 
        announcement_label = ctk.CTkLabel(change_button_page, text="After modfying password sucessfully, you will be logged out from account.", font=(font_style, 14, "bold"))
        announcement_label.place(x=15, y=135)

        # Create current password label and entry box
        current_password_label = ctk.CTkLabel(change_button_page, text="Current Password", font=(font_style, 18))
        current_password_label.place(x=150, y=160)

        current_password_entry = ctk.CTkEntry(change_button_page, width=300, show="*")
        current_password_entry.place(x=80, y=190)
        ...
        current_password_shown_check = ctk.CTkCheckBox(change_button_page, text="Show Password", 
                                        ...
                                        font=(font_style, 10))
        current_password_shown_check.place(x=400, y=190)


        # Create new password label and entry box
        new_password_label = ctk.CTkLabel(change_button_page, text="New Password", font=(font_style, 18))
        new_password_label.place(x=170, y=250)

        new_password_entry = ctk.CTkEntry(change_button_page, width=300, show="*")
        new_password_entry.place(x=80, y=280)
        ...
        new_password_shown_check = ctk.CTkCheckBox(change_button_page, text="Show Password", 
                                        ...
                                        font=(font_style, 10))
        new_password_shown_check.place(x=400, y=280)


        # Create confirm new password label and entry box
        confirm_new_password_label = ctk.CTkLabel(change_button_page, text="Confirm New Password", font=(font_style, 18))
        confirm_new_password_label.place(x=140, y=350)

        confirm_new_password_entry = ctk.CTkEntry(change_button_page, width=300, show="*")
        confirm_new_password_entry.place(x=80, y=380)
        ...
        confirm_password_shown_check = ctk.CTkCheckBox(change_button_page, text="Show Password", 
                                        ...
                                        font=(font_style, 10))
        confirm_password_shown_check.place(x=400, y=380)     
    ...
    def end_user_logout_button_function(user_email):
        ...
        if confirm == "yes":
            ...
            # If user cancels logout, return to the end user dashboard (home page)
            end_user_dashboard_function(user_email)
